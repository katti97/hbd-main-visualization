...
const loadCache = () => {
  try {
    return JSON.parse(localStorage.getItem("hb_cache") || "{}");
  } catch {
    return {};
  }
};
...
const saveCache = (cache) => {
  localStorage.setItem("hb_cache", JSON.stringify(cache));
};

const getCached = (queryText) => {
  const cache = loadCache();
  return cache[queryText];
};

const storeCached = (queryText, data) => {
  const cache = loadCache();
  cache[queryText] = data;
  saveCache(cache);
};
...
export default function HummingBirdAnalytics() {
  ...
  const executeQuery = async (queryText) => {
    ...
    const cached = getCached(queryText);
    if (cached) {
      console.log("⚡ Loaded from cache:", cached);
      setResult(cached);
      // Update history order (no API call)
      setHistory(prev => [
        {
          query: queryText,
          timestamp: new Date().toISOString(),
          recordCount: cached.raw_results?.length || 0
        },
        ...prev.filter(h => h.query !== queryText)
      ].slice(0, 5));
      return; // NO backend call
    }

    // 2️⃣ Not cached → call backend
    setIsLoading(true);
    setResult(null);
    try {
      const response = await fetch(`${API_BASE_URL}/api/query`, {
  ...
  const downloadChart = async (index) => {
    ...
    try {
      ...
      link.href = dataUrl;
      link.download = `chart-${index + 1}.jpg`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    } catch (err) {
      console.error("Chart export failed:", err);
      alert("Failed to download chart JPG.");
    }
  };

  const downloadAllCharts = async () => {
    if (!result || !result.suggested_charts) return;
    for (let i = 0; i < result.suggested_charts.length; i++) {
  ...
  const downloadXLSX = () => {
    if (!result || !result.raw_results || result.raw_results.length === 0) {
      alert("No data available to download.");
      return;
    }

    try {
      const worksheet = XLSX.utils.json_to_sheet(result.raw_results);
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, "Results");
      XLSX.writeFile(workbook, "data_results.xlsx");
    } catch (err) {
  ...
  const exampleQueries = [
    { icon: LineChart, text: 'Show total number of bookings' },
    { icon: TrendingUp, text: 'What is the total revenue by city?' },
    { icon: PieChart, text: 'Show average rating by property' },
    { icon: Activity, text: 'Show booking trends by month' },
  ];
                  ...
                  result.raw_results.length > 0 && (
                    ...
                    >
                      ...
                      <div style={{
                        display: 'grid',
                        gridTemplateColumns:
                          result.suggested_charts.length === 1
                            ? '1fr'
                        ...
                        {result.suggested_charts.map((chart, idx) => {
                          ...
                          // Normalize data
                          const numericData = chartData.slice(0, 20).map(row => ({
                            x: String(row[xCol]),
                            y: Number(row[yCol]) || 0,
                          }));

                          const maxValue = Math.max(...numericData.map(d => d.y));
                          const minValue = Math.min(...numericData.map(d => d.y));
                          const range = maxValue - minValue || 1;

                          const isLineChart = String(chart.type || '').toLowerCase() === "line";

                          // Basic SVG layout for line chart
                          const width = 360;
                          const height = 180;
                          ...
                          const points = numericData
                            .map((d, i) => {
                              const x =
                                paddingLeft +
                                (innerWidth * i) /
                                Math.max(numericData.length - 1, 1);
                              const y =
                                paddingTop +
                                innerHeight * (1 - (d.y - minValue) / range);
                              return `${x},${y}`;
                            })
                            .join(" ");
                      ...
                      <div style={{ textAlign: "center", marginTop: "1.5rem" }}>
                        ...
                        >
                          ⬇ Download All Charts (each as JPG)
                        </button>
                      </div>
                    </div>
                  )}
